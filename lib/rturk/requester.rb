...
module RTurk
  class Requester
    ...
    def initialize(aws_id, aws_key)
      @rturk_instance = RTurkInstance.new aws_id, aws_key if aws_id && aws_key
    end
  ...
  def self.Request(aws_id, aws_key, *args)
    requester = RTurk::Requester.new aws_id, aws_key
